...
namespace app\moudels\video\business;


use app\moudels\video\model\Video as VideoModel;
...
class Video implements RmtopBusiness
{
    ...
    static function rmtop_struct_ini($action, $colum_id)
    {
        ...
        switch ($action){
            case "index":
                return self::get_video_list($colum_id,'',20);
            case "edit";
                return  self::get_video_list(request()->param()['id'],'*');
            default:
                return  [];
        }
    }
    ...
    static function rmtop_struct_consumer($action, $params)
    {
        ...
        switch ($action){
            case "list":
                return self::get_video_list($params['col_id'],$params['keyword'],$params['limit']);
            case "info";
                return self::get_video_info($params['col_id'],$params['field']);
            default:
                return  [];
        }
    }
     ...
     */
    static function get_video_list(int $col_id,string  $searchKey,int $count = 10): \think\Paginator
    {
        $where = $col_id?"col_id = $col_id":1;
        return VideoModel::where($where)->whereLike('title',"%$searchKey%")->order('id','desc')->paginate(['list_rows'=>$count,'query' => request()->param()])->each(function (&$item){
     ...
     * @throws ModelNotFoundException
     */
    static function get_taglib_video_list(int $cid, string $filed, string $searchKey, int $limit = 10, string $orderBy ='create_time desc'){
        $list = VideoModel::where(['col_id'=>$cid])->whereLike('title',"%$searchKey%")->field($filed)->order($orderBy)->page(input('page')??1,$limit)->select();
     ...
     * @param $data
     * @return VideoModel|bool|\think\Model
     * @throws DataNotFoundException
     * @throws DbException
     * @throws ModelNotFoundException
     */
    static  function add_edit_vedio(string $type,$data){
        $data['content'] = $data['editorValue'];
        if($data['imglist']){$data['img_lists'] = implode(',',$data['imglist']);}
        if($type == 'add'){
           return  VideoModel::create($data);
        }else if($type == "edit"){
            $info = VideoModel::find($data['id']);
     ...
     * @throws ModelNotFoundException
     */
    static function get_video_info($id,string $filed = ''){
        if(!is_numeric($id)){
        ...
        if($filed == "img_lists"){
            $imglist =  VideoModel::where($condition)->value("img_lists");
            return  explode(',',$imglist);
        }else if($filed == 'view_path'){
            //若设置了单页单独模板
            $find  = VideoModel::where($condition)->find();//单独的详情设置模板页面
            ...
            VideoModel::where($condition)->update(['views'=>	Db::raw('views+1')]);
            return VideoModel::where($condition)->field($filed)->find();
        }
    }
     ...
     * @return bool
     */
    static function delete_vedio(int $id): bool
    {
        return VideoModel::where(['id'=>$id])->delete();
    }
     ...
     * @return bool
     */
    static function delete_vedio_by_col_id(int $col_id): bool
    {
        return  VideoModel::where(['col_id'=>$col_id])->delete();
    }
}